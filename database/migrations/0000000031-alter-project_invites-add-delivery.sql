...
--
-- Nothing may follow the last statement below except a newline. The migration
-- runner rewrites files "to be idempotent" by appending a semicolon to the last
-- line without checking whether that line is a comment, so a file ending in
-- prose comes back from a migrate run with a `;` glued inside the comment.
